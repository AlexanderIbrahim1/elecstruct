...
TEST_CASE("gaussian product")
{
    SECTION("example from desmos")
    {
        ...
        const auto info0 = elec::GaussianContractionInfo {1.0, 1.0 / 2.0};
        const auto info1 = elec::GaussianContractionInfo {1.0, 1.0 / 3.0};
        ...
        const auto expected_exponent = info0.exponent_coeff + info1.exponent_coeff;
        ...
        REQUIRE_THAT(gauss.contraction_coeff, Catch::Matchers::WithinAbs(expected_coefficient, ABS_TOL));
        REQUIRE_THAT(gauss.exponent_coeff, Catch::Matchers::WithinAbs(expected_exponent, ABS_TOL));
    }
}
...
TEST_CASE("unnormalized overlap integrals")
{
    ...
    SECTION("s-function with itself")
    {
        ...
        const auto gauss0 = elec::GaussianContractionInfo {coefficient0, exponent0};
        const auto gauss1 = elec::GaussianContractionInfo {coefficient1, exponent1};
        ...
        const auto unorm_overlap_x = elec::unnormalized_overlap_integral_1d(
            {angmom0.x, gauss0.exponent_coeff, centre0.x},
            {angmom1.x, gauss1.exponent_coeff, centre1.x},
            new_centre.x
        );

        const auto unorm_overlap_y = elec::unnormalized_overlap_integral_1d(
            {angmom0.y, gauss0.exponent_coeff, centre0.y},
            {angmom1.y, gauss1.exponent_coeff, centre1.y},
            new_centre.y
        );

        const auto unorm_overlap_z = elec::unnormalized_overlap_integral_1d(
            {angmom0.z, gauss0.exponent_coeff, centre0.z},
            {angmom1.z, gauss1.exponent_coeff, centre1.z},
            new_centre.z
        );
        // clang-format on
    ...
    SECTION("p-function with itself")
    {
        ...
        const auto expected_unorm_overlap_x = 1.0 / (2.0 * (gauss0.exponent_coeff + gauss1.exponent_coeff));
